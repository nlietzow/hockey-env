...
class HockeyEnv(gym.Env, EzPickle):
    ...
    def _get_info(self):
        ...
        is_success = self.winner == 1 if self.done else None
        return {
...
class HockeyEnvBasicOpponent(HockeyEnv):
    def __init__(self, weak=False, mode=Mode.NORMAL):
...
class HockeyEnvAIOpponent(HockeyEnv):
    def __init__(
        self,
        model_class: Type[OffPolicyAlgorithmJax],
        checkpoint: Path,
        mode=Mode.NORMAL,
    ):
        super().__init__(mode=mode, keep_mode=True)
        self.opponent = model_class.load(checkpoint)
        self.action_space = spaces.Box(-1, +1, (4,), dtype=np.float32)

    def step(self, action):
        ob2 = self.obs_agent_two()
        a2, _ = self.opponent.predict(ob2)
        action2 = np.hstack([action, a2])
        return super().step(action2)

    def update_opponent(self, params: TensorDict):
        self.opponent.set_parameters(params)
